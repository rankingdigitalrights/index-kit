...
function addIndicatorGuidance(
  ...
  if (includeRGuidanceLink) {
    ...
    cell = sheet
      .getRange(row, col)
      .setValue(
        "▶ Please read the indicator-specific guidance and discussions before starting the research!"
      )
      .setFontWeight("bold")
      .setFontSize(9)
      .setHorizontalAlignment("left")
  ...
  if (includeRGuidanceLink) {
    ...
    cell = sheet
      .getRange(row, col, 1, maxColHeadings)
      .merge()
      .setWrap(true)
  ...
  sheet
    .getRange(tempStartRow, 1, row - tempStartRow + 1, numberOfColumns)
    .setBackground("WhiteSmoke");

  sheet
    .getRange(row, activeCol, 1, numberOfColumns)
...
function addSubStepHeader(
  ...
  for (
    let serviceNr = 1;
    serviceNr < companyNumberOfServices + 3;
    serviceNr++
  ) {
...
function addEvaluationDropdown(
  ...
  let rule = SpreadsheetApp.newDataValidation()
    .requireValueInList(currentStep.components[stepCNr].dropdown)
    .build();
...
function addBinaryEvaluation(
  ...
  for (
    ...
    // (((companyNumberOfServices+2)*nrOfIndSubComps)+1)
...
function addComparisonYonY(
  ...
  let colMax = columnToLetter(
    2 + (companyNumberOfServices + 2) * nrOfIndSubComps
  );
  ...
  let range = sheet.getRange(
    activeRow,
    2,
    currentIndicator.elements.length,
    2 + (companyNumberOfServices + 2) * nrOfIndSubComps
  );
